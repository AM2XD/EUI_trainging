...
s32 Perform_Operation(s16 operand1, s16 operand2, char operator)
{
    s32 result;
...
void CALCULATE(char  element)
{
    ...
    else if (element == '=')
    {
        ...
        if(Get_Priority(OPERATION[Local_u8OperatorIndex]) ==2 && Local_u8OperatorIndex > -1)
        {
            ...
            Local_u16Result = Perform_Operation(NUMBERS_NUMBER[Last_u8Index -1] , NUMBERS_NUMBER[Last_u8Index] ,OPERATION[Local_u8OperatorIndex] );


            //don't forget to Local_u8OperatorIndex--
            Local_u8NumberIndex-=1;

            NUMBERS_NUMBER[Local_u8NumberIndex] = Local_u16Result;
        ...
        {
            Local_u16Result = Perform_Operation(NUMBERS_NUMBER[0] , NUMBERS_NUMBER[j+1] ,OPERATION[j] );
            NUMBERS_NUMBER[0] = Local_u16Result;
